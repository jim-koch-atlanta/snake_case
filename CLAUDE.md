...
- Bench: 9 BE + 3 IR → 22 roster spots, 19 live picks per team (3 keepers each)
...
- Recommendation = maximize `value(now) − E[best available at that position at my next pick]`, subject to the **legal-lineup feasibility guard**: `picks_remaining − mandatory_unfilled_slots` (QB, K, 2DL, 2LB, 2DB) must never go negative; surface it in the UI, red at 0.
